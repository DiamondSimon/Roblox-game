...
Rush=lua.execute((ROOT/'src/shared/Config/RewardsConfig.lua').read_text())
...
def simulate(profile,double,seed,loss=0):
 ...
 for t in range(1,3601):
  balance+=E.income(sum(incomes[x] for x in inventory),levels['Income'],double,0)*(Rush.RushMultiplier if t%Rush.RushPeriod>=Rush.RushPeriod-Rush.RushDuration else 1)
