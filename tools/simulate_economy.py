...
def simulate(profile,double,seed,loss=0):
 ...
 for t in range(1,3601):
  ...
  if t>=first and (t-first)%interval==0:
   ...
   candidates=[x for x in candidates if M.ById[x].RequiredRebirths==0]
   item=max(candidates,key=lambda x:incomes[x]) if candidates else None
   if item is not None and rng.random()>=loss:
